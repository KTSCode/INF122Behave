...
public class UserController
{
  ...
  public String executeAction(Scanner keyboard){
    ...
    switch (user.getClass().getName()){
      case "Parent":
        Parent rent = (Parent) user;
        ArrayList<Child> children = new ArrayList<>(rent.childList.values());
        ...
        switch (menuOpt){
          case 1: // Add Child FIXME Wet code copied from Create user method
            ...
            switch (Integer.parseInt(keyboard.nextLine())){
              case 1:
                rent.addChild(cname, "Positive");
                break;
              case 2:
                rent.addChild(cname, "Negative");
                break;
              default:
          ...
          default:
            if(menuOpt > 0 && menuOpt < (children.size() + 3)){
              out = out + parentMenu(rent, children.get(menuOpt - 3), keyboard);
            }else return "!Invalid Input!";
        }
        break;
      case "Child":
        return childMenu((Child) user, keyboard);
      default:
        return "!user type does not have mainMenu!\n";
    }
      // Child Menu Switch
    return out;
  }
  ...
  public String childMenu(Child child, Scanner keyboard){
    String out = "";
    System.out.println(child.name + " Menu:\n1. View Status\n2. Redeem Reward");
    switch (Integer.parseInt(keyboard.nextLine())){
      case 1:
